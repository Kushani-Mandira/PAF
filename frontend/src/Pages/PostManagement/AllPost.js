...
function AllPost() {
  ...
  useEffect(() => {
    const fetchFollowedUsers = async () => {
      ...
      if (userID) {
        try {
          const response = await axios.get(`http://localhost:8081/user/${userID}/followedUsers`);
  ...
  const handleLike = async (postId) => {
    ...
    try {
      const response = await axios.put(`http://localhost:8081/posts/${postId}/like`, null, {
        params: { userID },
      });
  ...
  const handleFollowToggle = async (postOwnerID) => {
    ...
    try {
      if (followedUsers.includes(postOwnerID)) {
        // Unfollow logic
        await axios.put(`http://localhost:8081/user/${userID}/unfollow`, { unfollowUserID: postOwnerID });
        setFollowedUsers(followedUsers.filter((id) => id !== postOwnerID));
      } else {
        // Follow logic
        await axios.put(`http://localhost:8081/user/${userID}/follow`, { followUserID: postOwnerID });
  ...
  const handleAddComment = async (postId) => {
    ...
    try {
      const response = await axios.post(`http://localhost:8081/posts/${postId}/comment`, {
        userID,
        content,
      });
  ...
  const handleDeleteComment = async (postId, commentId) => {
    const userID = localStorage.getItem('userID');
    try {
      await axios.delete(`http://localhost:8081/posts/${postId}/comment/${commentId}`, {
        params: { userID },
      });
  ...
  const handleSaveComment = async (postId, commentId, content) => {
    try {
      const userID = localStorage.getItem('userID');
      await axios.put(`http://localhost:8081/posts/${postId}/comment/${commentId}`, {
        userID,
        content,
      });
                    ...
                    {post.media.slice(0, 4).map((mediaUrl, index) => (
                      ...
                      >
                        {mediaUrl.endsWith('.mp4') ? (
                          <video controls>
                            <source src={`http://localhost:8081${mediaUrl}`} type="video/mp4" />
                            Your browser does not support the video tag.
                          </video>
                        ) : (
                          <img src={`http://localhost:8081${mediaUrl}`} alt="Post Media" />
                        )}
      ...
      >
        ...
        {selectedMedia && selectedMedia.endsWith('.mp4') ? (
          <video controls className="modal-media">
            <source src={`http://localhost:8081${selectedMedia}`} type="video/mp4" />
            Your browser does not support the video tag.
          </video>
        ) : (
          <img src={`http://localhost:8081${selectedMedia}`} alt="Full Media" className="modal-media" />
        )}
      </Modal>
    </div>
  );
